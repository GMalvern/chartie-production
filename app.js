...
function setAccent(hex) {
  ...
  const url =
    ...
    "<path d='M2 9 Q 22 2, 42 8 T 98 8' fill='none' stroke='" +
    acc +
...
function refreshHighlights() {
  ...
  els.forEach((el) => {
    ...
    el.classList.add(
      style === "brush" ? "hl-brush" : style === "none" ? "hl-none" : "hl-clean"
    );
  });
}
...
function distribute(byBlank) {
  const areaIds = Array.from(qtRows.querySelectorAll("textarea")).map(
    (t) => t.id
  );
...
function applyStyling() {
  ...
  // sticky
  const showSticky = toggleSticky.checked;
  stickyBox.classList.toggle("hidden", !showSticky);
  stickyBox.setAttribute("aria-hidden", String(!showSticky));
...
function renderSimple(layout) {
  ...
  if (layout === "simple-2col") {
    ...
    row.appendChild(
      makeCard(
        "",
        `<div class="whitespace-pre-wrap">${v.col1 || ""}</div>`
      )
    );
    row.appendChild(
      makeCard(
        "",
        `<div class="whitespace-pre-wrap">${v.col2 || ""}</div>`
      )
    );
  ...
  if (layout === "simple-3col") {
    ...
    row.appendChild(
      makeCard(
        "",
        `<div class="whitespace-pre-wrap">${v.col3 || ""}</div>`
      )
    );
    contentEl.appendChild(row);
  }
  ...
  if (layout === "simple-title-sub-3") {
    if (v.sub) {
      contentEl.appendChild(makeCard("", `<div class="pill">${v.sub}</div>`));
    }
  ...
  if (layout === "simple-quote") {
    const quote = `<div class="quote">“${(v.quote || "").replace(
      /^["“]|["”]$/g,
      ""
    )}”</div>`;
    const by = v.by
      ? `<div class="mt-2 text-slate-600">— ${v.by.replace(
          /^—\s*/,
          ""
        )}</div>`
      : "";
  ...
  if (layout === "simple-def-callout") {
    const top = `
      <div class="inline-block px-3 py-1 rounded-lg border bg-white/80 mb-2">
        <span class="marker-h">
          <span class="hl hl-clean">${v.term || "Term"}</span>
        </span>
      </div>
    `;
    ...
    const ex = exArr.length
      ? `<div>
          <span class="text-xs font-semibold text-slate-600">Examples</span>
  ...
  if (layout === "simple-image") {
    const img = v.img
      ? `<img src="${v.img}" alt="" class="w-full h-auto rounded-lg border mb-2">`
...
function renderCompare(obj) {
  ...
  wrap.appendChild(
    makeCard(
      obj.leftTitle || "Side A",
      `<ul class="pretty-list list-disc pl-6">${(obj.leftBullets || [])
        .map(
          (b, i) =>
            `<li>${toggleEmoji.checked ? emojiFor(i) : ""}${b}</li>`
        )
  ...
  wrap.appendChild(
    makeCard(
      obj.rightTitle || "Side B",
      `<ul class="pretty-list list-disc pl-6">${(obj.rightBullets || [])
        .map(
          (b, i) =>
            `<li>${toggleEmoji.checked ? emojiFor(i) : ""}${b}</li>`
        )
...
function renderCauseEffect(obj) {
  ...
  wrap.appendChild(
    makeCard(
      "Causes",
      `<ul class="pretty-list list-disc pl-6">${(obj.causes || [])
        .map(
          (b, i) =>
            `<li>${toggleEmoji.checked ? emojiFor(i) : ""}${b}</li>`
        )
  ...
  wrap.appendChild(
    makeCard(
      "Effects",
      `<ul class="pretty-list list-disc pl-6">${(obj.effects || [])
        .map(
          (b, i) =>
            `<li>${toggleEmoji.checked ? emojiFor(i) : ""}${b}</li>`
        )
...
function renderTimelineH(obj) {
  ...
  events.forEach((ev) => {
    row.appendChild(
      makeCard(ev.title, `<p class="text-sm">${ev.text}</p>`)
    );
  });
...
function formatMath(text) {
  ...
  // Simple numeric fractions like 3/4 (but not dates)
  out = out.replace(
    /(^|[^\d])(\d+)\s*\/\s*(\d+)(?!\d)/g,
...
// ----- MathEx renderer (with per-step work lines) -----
function renderMathEx(obj) {
  ...
  const stepsHtml = (obj.steps || []).map((step) => {
    ...
    const textHtml = text
      ? `<div class="step-main">${formatMath(text)}</div>`
      : "";
    const workHtml = work
      ? `<div class="step-workline mathface ${
          toggleHand.checked ? "math-hand" : ""
        }">${formatMath(work)}</div>`
      : "";

    return `<li>${textHtml}${workHtml}</li>`;
  }).join("");

  contentEl.appendChild(
    makeCard(
      "Steps",
      `<ol class="steps-list list-decimal pl-6">${stepsHtml}</ol>`
    )
  );
...
- steps (array) where each item is EITHER:
    - a plain string (for backward compatibility), OR
    - an object with:
    ...
    schema = {
      ...
      properties: {
        ...
        steps: {
          type: "array",
          items: {
            // allow generic objects; we’ll handle strings in renderMathEx
            type: "object",
            properties: {
              text: { type: "string" },
              work: { type: "string" },
