...
// Hide the Format pill completely (no more gray chip)
if (formatBadge) {
...
function renderSimple(layout){
  ...
  // We NO LONGER show "Format: ..." anywhere
  if (formatBadge) formatBadge.textContent = '';
  ...
  if(layout==='simple-def-callout'){
    ...
    const ex = exArr.length
      ? `<div><span class="text-xs font-semibold text-slate-600">Examples</span><ul class="pretty-list list-disc pl-6">${exArr.map((e,i)=>`<li>${toggleEmoji.checked?emojiFor(i):''}${e}</li>`).join('')}</ul></div>`
      : '';
...
function renderCompare(obj){
  ...
  wrap.appendChild(
    makeCard(
      ...
      `<ul class='pretty-list list-disc pl-6'>${(obj.leftBullets||[]).map((b,i)=>`<li>${toggleEmoji.checked?emojiFor(i):''}${b}</li>`).join('')}</ul>`
    )
  );
  wrap.appendChild(
    makeCard(
      obj.rightTitle||'Side B', 
      `<ul class='pretty-list list-disc pl-6'>${(obj.rightBullets||[]).map((b,i)=>`<li>${toggleEmoji.checked?emojiFor(i):''}${b}</li>`).join('')}</ul>`
    )
  );
...
function formatMath(text) {
  ...
  // Only matches pure number/number so dates like 3/4/2024 don't get weird.
...
// MathEx renderer (with mini worked line under each step)
function renderMathEx(obj){
  ...
  // Steps + mini worked line under each (if available)
  const examples = obj.example || [];
        ...
        .map((s, idx) => {
          ...
          return `
            <li>
              ...
              ${exLine ? `
                <div class="step-math mathface ${toggleHand.checked?'math-hand':''}">
                  ${formatMath(exLine)}
                </div>
              ` : ''}
            </li>
  ...
  // Full Worked Example lines (keep the full version at the bottom)
  contentEl.appendChild(
    makeCard(
      'Worked Example',
...
function renderByLayout(layout, data){
  ...
  // Kill the Format pill text here too
  if (formatBadge) formatBadge.textContent = '';
